...
#if !defined(WIN32) && LWS_POSIX && !defined(LWS_WITH_ESP32) && !defined(LWS_WITH_XRADIO)
	size_t len;
#endif
	int n;
...
#if LWS_POSIX && !defined(LWS_WITH_ESP32) && !defined(LWS_WITH_XRADIO)
		    ...
		    const char *other_headers, int other_headers_len)
{
	...
	if (ranges >= 2) { /* multipart byteranges */
		 ...
		 *
		 *  - (n + 1) x "_lws\r\r\n"
		 *  - n x Content-Type: xxx/xxx\r\r\n
		 *  - n x Content-Range: bytes xxx-yyy/zzz\r\r\n
		 *  - n x /r/n
		...
		while (lws_ranges_next(rp)) {
			...
			total_content_length +=
				6 /* header _lws\r\r\n */ +
				/* Content-Type: xxx/xxx\r\r\n */
				14 + strlen(content_type) + 2 +
				/* Content-Range: xxxx\r\r\n */
				15 + n + 2 +
				2; /* /r/n */
		}
