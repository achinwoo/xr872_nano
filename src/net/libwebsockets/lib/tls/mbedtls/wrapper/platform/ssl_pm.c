...
static int ssl_pm_reload_crt(SSL *ssl)
{
    ...
    if (ret) {
        SSL_DEBUG(SSL_PLATFORM_ERROR_LEVEL, "mbedtls_ssl_conf_own_cert() return -0x%x\r\n", -ret);
        ret = -1;
    }
...
static int mbedtls_handshake( mbedtls_ssl_context *ssl )
{
    ...
    while (ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER) {
        ...
        SSL_DEBUG(SSL_PLATFORM_DEBUG_LEVEL, "ssl ret %d state %d\r\n", ret, ssl->state);
...
int ssl_pm_handshake(SSL *ssl)
{
    ...
    if (ret == MBEDTLS_ERR_SSL_WANT_READ || ret == MBEDTLS_ERR_SSL_WANT_WRITE) {
        SSL_DEBUG(SSL_PLATFORM_ERROR_LEVEL, "mbedtls_ssl_handshake() return -0x%x\r\n", -ret);
